...
public class RemoveListModelPropertyTest extends AbstractListModelPropertyTest {
	...
	public void testRemoveIndexedPropertyAtEnd() {
		...
		assertSame(ELEMENT_0_ELEMENT_1, ListModelProperties.ELEMENTLIST.remove(this.model, context));
		assertEquals(1, this.model.get(0).size());
		assertSame(ELEMENT_0_ELEMENT_0, this.model.get(0).get(0));
		
		assertSame(ELEMENT_0_ELEMENT_0, ListModelProperties.ELEMENTLIST.remove(this.model, context));
		assertEquals(0, this.model.get(0).size());
		
		assertEquals(null, ListModelProperties.ELEMENTLIST.remove(this.model, context));
	}

	@Test
	public void testRemoveIndexedPropertyAtIndex() {
		DefaultContext context = DefaultContext.of(PropertyIndex.of(ListModelProperties.MODEL, 0));
		assertSame(ELEMENT_0_ELEMENT_0, ListModelProperties.ELEMENTLIST.removeAt(this.model, 0, context));
		assertEquals(1, this.model.get(0).size());
	...
	public void testRemoveIndexedPropertyIntermediateNull() {
		...
		DefaultContext context = DefaultContext.of(PropertyIndex.of(ListModelProperties.MODEL, 0));
		ListModelProperties.ELEMENTLIST.remove(this.model, context);
	}

	@Test(expected = OutboundPropertyPathException.class)
	public void testRemoveIndexedPropertyOutOfBounds() {
		DefaultContext context = DefaultContext.of(PropertyIndex.of(ListModelProperties.MODEL, 0));
		ListModelProperties.ELEMENTLIST.removeAt(this.model, 2, context);
	}
}
