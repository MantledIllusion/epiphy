...
/**
 * Exception that might be thrown if a writing operation is executed on a property that has
 * no {@link Setter} provided for it, so it is read only.
 */
public class ReadonlyPropertyException extends NullPointerException {
	...
	public ReadonlyPropertyException(Property<?, ?> property) {
		super("Unable to perform writing operation on property '" + property
				+ "'; the property has no setter provided for it.");
	}
}
